 ...
 */
conferenceAppModule.controller('conferenceVideoPlayer', ['$rootScope', '$scope', '$http', '$interval', '$location', '$window', 'AppConstants',
            ...
            var callback = function(evt) {
                switch(evt.type) {
                    ...
                    case "buffered":
                        ...
                        //document.getElementById("mainFormProgress2").value = percent;
                        break;

                    case "progress":
                        /!*var current = evt.progress;
                        var total = evt.total;
                        var prog = document.getElementById("mainFormProgress");
                        prog.value = Math.floor((current * 100)/total);
                        document.getElementById("totalTime").innerHTML = getFormattedTime(total);
                        document.getElementById("currentTime").innerHTML = getFormattedTime(current);
                        var offset = -35 + Math.floor((prog.offsetWidth * current)/total);
                        document.getElementById("seeker").style.left = offset+"px";*!/
                        break;

                }
            };

            var app = new WebRtcPlayer("https://nstl.socialvid.in/videos/981921861291/223448e88947b802/master.json",
                document.getElementById("webrtcPlayerContainer"), callback);
